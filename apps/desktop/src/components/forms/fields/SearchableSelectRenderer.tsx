...
export function SearchableSelectRenderer({ field, disabled }: Props) {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["field-search", field.id, query],
    queryFn: async () => {
      if (!source || query.length < (source.minChars ?? 2)) return { items: [] };
      const res = await apiClient.get<{ items: SearchItem[] }>(source.endpoint, {
        params: { [source.searchParam]: query, pageSize: source.pageSize ?? 20 },
      });
  ...
  // New record inline creation
  const handleCreateNew = () => {
    // Navigate to the corresponding creation action
      ...
      {isOpen && !disabled && (
        <>
          ...
          <div className="absolute top-full left-0 right-0 z-50 mt-1 rounded-lg border border-border bg-popover shadow-xl overflow-hidden">
            {items.length > 0 ? (
            ...
            ) : query.length >= (source?.minChars ?? 2) && !isFetching ? (
              ...
              <div className="px-3 py-3 text-center">
                <p className="text-xs text-muted-foreground">
                  {query.length < (source?.minChars ?? 2)
                    ? `Mindestens ${source?.minChars ?? 2} Zeichen eingeben...`
                    : "Suchen..."}
                </p>
              </div>
            )}
