...
def pruefe_zeile(zeile: dict, aktive_bestellungen: list[dict],
                 zeitfenster_tage: int = 30,
                 aws_mit_wareneingang: set[str] | None = None) -> dict:
    """Eine Bedarfszeile gegen Historie pruefen.

    Returns dict mit:
      bucket: 'neu' | 'schon_bestellt' | 'teilbedarf' | 'angekommen'
      ...
      match_typ: 'aw_artikel' | 'kunde_artikel_zeitfenster' | 'angekommen' | None

    Spec §6: 'angekommen' wenn die AW bereits in einer Beschaffung mit
    Status 'eingegangen' verlinkt ist (set wird vom Caller via
    ...
    # §6: Wareneingang-Check VOR Historie-Match — wenn die Paletten
    # bereits angekommen sind, ist der Bedarf erfüllt.
    if aws_mit_wareneingang and aw and aw in aws_mit_wareneingang:
        return {
            "bucket": "angekommen",
            "delta_menge": 0,
            "gemachte_menge": menge_neu,
            "historien_ids": [],
            "match_typ": "angekommen",
        }
...
def pruefe_import(zeilen: list[dict],
                  ...
                  zeitfenster_tage: int = 30,
                  aws_mit_wareneingang: set[str] | None = None) -> dict:
    """Komplette Import-Pruefung mit 4 Buckets (Spec §6 + §8):
      neu          → wird optimiert
      schon_bestellt → übersprungen (AW in Historie/Beschaffung)
      teilbedarf   → nur Delta in Optimierung
      angekommen   → übersprungen (Paletten bereits eingegangen)
    """
    neu, schon, teil, angekommen = [], [], [], []
    fuer_opt = []
    for z in zeilen:
        info = pruefe_zeile(z, aktive_bestellungen, zeitfenster_tage,
                              aws_mit_wareneingang=aws_mit_wareneingang)
        bucket = info["bucket"]
        if bucket == "neu":
            neu.append((z, info))
            fuer_opt.append(z)
        elif bucket == "angekommen":
            angekommen.append((z, info))
